...
# Path file disesuaikan dengan lokasi file disimpan.
data = read.csv("G:/RYCAHAYA/ITS/Informatika/Mata Kuliah/Semester 4/Probabilitas dan Statistik (C)/Praktikum/data_nomor_1.csv")
data
...
# C
# Nilai kritis dari t-table, dengan df = 8, alpha = 0.05
qt(p = .05/2, df=8, lower.tail = FALSE)

# Didapatkan nilai kritis sebesar 2.306004. 
# Dari hal ini dapat disimpulkan nilai t hitung > t kritis, yakni 7.6525 > 2.306004
...
# Karena n > 30, maka gunakan tabel Z.
...
# C
# Karena nilai z dari perhitungan berada di dalam area "tidak tolak H0",
# maka dapat disimpulkan bahwa sample tidak memiliki cukup bukti untuk menolak H0 (menerima H0).
...
# Variabel
data4 = read.delim("G:/RYCAHAYA/ITS/Informatika/Mata Kuliah/Semester 4/Probabilitas dan Statistik (C)/Praktikum/onewayanova.txt", header = TRUE, sep = "\t", dec = ".")
data4
...
# keberadaan outlier ditandakan dengan adanya lingkaran  (data) yang berada diluar kotak grupnya.
# Dalam kasus ini tidak ada lingkaran di luar grup,
# sehingga bisa dikatakan bahwa tidak ada outlier.
...
# dari bartlett test, didapatkan nilai p-value sebesar 0.8054,
# di mana nilai ini lebih besar dari pada alpha (0.05),
# sehingga menerima H0 dalam arti lain, tidak ada bukti yang cukup  untuk menunjukkan
...
# Karena nilai p-value lebih besar dari alpha (0.05),
# maka menerima H0.
...
# Dari Tukey HSD, terdapat 2 perbedaan data yang lebih kecil dari alpha (0.05)
...
# Hasil dari kedua metode tersebut terlihat berlawanan.
# Namun, pada dasarnya kedua metode ini memang memiliki tujuannya masing-masing.
# Sehingga, hasil dari kedua metode ini memang tidak bisa dibandingkan.

# F
#titik
data.mod=data.frame(fitted=fitted(data4.regression),residuals=resid(data4.regression),treatment=data4$Group)
ggplot(data.mod,aes(fitted,Length,color=treatment))+geom_point()
...
# Memuat dan mengecek data
GTL <- read_csv("G:/RYCAHAYA/ITS/Informatika/Mata Kuliah/Semester 4/Probabilitas dan Statistik (C)/Praktikum/GTL.csv")
head(GTL)
...
# B
# Uji ANOVA dua arah
# Membuat variabel sebagai faktor dari ANOVA
...
# tabel dengan vektor, rata-rata, dan standar deviasi
...
# Menambahkan compact letter display ke tabel dengan mean dan sd
